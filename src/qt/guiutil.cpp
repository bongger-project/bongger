...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("bongger"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("bongger://"))
    {
        uri.replace(0, 11, "bongger:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bongger.lnk";
}
...
{
    return GetAutostartDir() / "bongger.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bongger-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bongger-qt [" + tr("command-line options") + "]                     " + "\n";
