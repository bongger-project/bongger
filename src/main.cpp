...
uint256 hashGenesisBlock("0x5875fbd7fb96cd71522bffc54f5983211a9d5969eb51f32674b5d5c76757d4fe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bongger: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bongger Signed Message:\n";
...
{
    // Bongger: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nDiffChangeTarget = 1; // Patch effective @ block 1

static const int64 nTargetTimespan = 42 * 60; // old retarget (42 minutes)
static const int64 nTargetTimespanNEW = 42 * 60; // Bongger: 42 minutes
static const int64 nTargetSpacing = 42; // Bongger: 42 seconds
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight==1) return 84000000 * COIN + nFees; // block 1 contains the 84M premine

    // Subsidy is cut in half every 500001 blocks, which will occur approximately every 8 months
    nSubsidy >>= (nHeight / 500001); // Litecoin: 500k blocks in ~8 months
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bongger: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Bongger: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5875fbd7fb96cd71522bffc54f5983211a9d5969eb51f32674b5d5c76757d4fe");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 21/April/2014 Stronger Than Ever, Boston Marathon Opens";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398090805;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2393246;
        ...
        assert(block.hashMerkleRoot == uint256("0xf73be2dac4cf76a3520292c589aef88404e1af0c344c21561363362f7dbd240b"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BonggerMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BonggerMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BonggerMiner(CWallet *pwallet)
{
    printf("BonggerMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bongger-miner");
    ...
    try { loop {
        ...
        printf("Running BonggerMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BonggerMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BonggerMiner, pwallet));
}
